...
-- Smart Ration Distribution System Database Schema - Initial Migration

-- 1. Create RationShops Table
...
-- 2. Create Families Table (Without FK to FamilyMembers yet)
CREATE TABLE IF NOT EXISTS Families (
    ...
    address TEXT,
    default_face_member INT,
    family_members_count INT DEFAULT 1,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 3. Create FamilyMembers Table
...
-- 4. Add Circular Foreign Key Constraint on Families
...
CREATE TABLE IF NOT EXISTS FaceData (
    ...
    family_id INT NOT NULL REFERENCES Families(family_id) ON DELETE CASCADE,
    face_embedding_vector TEXT NOT NULL, 
    face_image_path VARCHAR(255) NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 6. Create MonthlyRation Table
